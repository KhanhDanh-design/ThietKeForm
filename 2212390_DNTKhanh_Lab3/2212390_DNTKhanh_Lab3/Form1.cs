...
namespace _2212390_DNTKhanh_Lab3
{
    public partial class frmSinhVien : Form
    {
        ...
        {
            SinhVien sv = new SinhVien();
            sv.MaSo = lv.SubItems[0].Text;
            sv.HoTen = lv.SubItems[1].Text;
            sv.NgaySinh = DateTime.Parse(lv.SubItems[2].Text);
            sv.DiaChi = lv.SubItems[3].Text;
            sv.Lop = lv.SubItems[4].Text;
            sv.GioiTinh = false;
            if (lv.SubItems[5].Text == "Nam")
                sv.GioiTinh = true;
            List<string> cn = new List<string>();
            string[] s = lv.SubItems[6].Text.Split(',');
            foreach (string t in s)
                cn.Add(t);
            sv.ChuyenNganh = cn;
            sv.Hinh = lv.SubItems[7].Text;
            return sv;
        }
